...
# Get files
pf_filenames=( $(ls -d ../Nam-benchmarks/PF/*.qasm) )
Arithmetic_and_Toffoli_filenames=( $(ls -d ../Nam-benchmarks/Arithmetic_and_Toffoli/*.qasm) )
Quipper_Adder_filenames=( $(ls -d ../Nam-benchmarks/QFT_and_Adders/Adder*.qasm) )
QFT_filenames=( $(ls -d ../Nam-benchmarks/QFT_and_Adders/QFT*.qasm) )
...
do
    ...
    (time dune exec -- ./voqc_cli.exe -i ${filename} -o out.qasm -optimize-nam-lcr ${iter} -v) &> ${program_name}.txt
...
do
    ...
    currentTime=`date`
    printf "${CYAN}   + [${currentTime}] Running on ${filename}${NOCOLOR}\n"
    (time dune exec -- ./voqc_cli.exe -i ${filename} -o out.qasm -optimize-nam-light -v) &> ${program_name}.txt
    python parseOutput.py ${program_name}.txt >> QFT_and_Adders_results.csv
    rm -rf ${program_name}.txt
done